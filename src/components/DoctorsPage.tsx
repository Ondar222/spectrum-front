              ...
              {filteredDoctors?.map((doctor: ArchimedDoctor) => (
                <div key={`doctor-${doctor.id}`} className="bg-white rounded-lg shadow-lg overflow-hidden hover:shadow-xl transition-shadow flex flex-col h-full">
                  ...
                  <div className="p-4 sm:p-6 flex flex-col flex-grow">
                    ...
                    <p className="text-primary font-medium mb-3 text-sm sm:text-base">{doctor.type}</p>

                    <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm text-gray-600 mb-4 flex-grow">
                    ...
                    {doctor.info && (
                      <p className="text-gray-600 text-xs sm:text-sm mb-4 leading-relaxed line-clamp-2">{doctor.info}</p>
                    )}

                    <div className="flex space-x-1.5 sm:space-x-2 mt-auto">
                      <button 
                        onClick={() => handleAppointmentClick(doctor)}
                        className="flex-1 bg-primary hover:bg-primaryDark text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg font-medium transition-colors text-xs sm:text-sm"
                      >
                        Записаться
                      </button>
                      <Link 
                        to={`/doctors/${doctor.id}`}
                        className="px-3 sm:px-4 py-1.5 sm:py-2 border border-primary text-primary hover:bg-primary hover:text-white rounded-lg font-medium transition-colors text-xs sm:text-sm"
                      >
                        Подробнее
                      </Link>
                    </div>
