...
export default function PromotionsPage() {
  ...
  const listToRender = useMemo(() => {
    ...
    return [{
      ...
      description: 'Описание акции Центра SpectrUM. Для получения подробной информации обращайтесь по нашим контактам.',
      discountLabel: 'Акция',
      validUntil: '',
      imageUrl: '/www.jpg',
                    ...
                    <div key={promo.id} className="bg-white rounded-lg shadow-md overflow-hidden">
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="bg-teal text-white px-3 py-1 rounded-full text-sm font-medium">{promo.discountLabel}</span>
                          <h3 className="text-base font-semibold text-gray-900 text-left">{promo.title}</h3>
                        </div>
                      ...
                      {isOpen && (
                        <div id={`promo-body-${key}`} className="border-t border-gray-200">
                          {promo.imageUrl && (
                          ...
                          <div className="p-4">
                            <p className="text-gray-600 mb-3 leading-relaxed">{promo.description}</p>
                            <div className="flex items-center justify-between">
                              <span className="text-sm text-gray-500">{promo.validUntil ? `Действует до: ${promo.validUntil}` : ''}</span>
                              {promo.link ? (
                                <a href={promo.link} target="_blank" rel="noopener noreferrer" className="bg-teal text-white px-4 py-2 rounded-md font-medium hover:bg-teal/90 transition-colors">{promo.isVideo ? 'Смотреть видео' : 'Подробнее'}</a>
                              ) : (
                                <button className="bg-teal text-white px-4 py-2 rounded-md font-medium hover:bg-teal/90 transition-colors">Записаться</button>
                              )}
                            </div>
                          </div>
                        </div>
